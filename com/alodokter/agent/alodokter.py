...
class AlodokterAgent:
    def run(self):
        current_app.logger.info('[AlodokterMonitor] running...')
        client = MongoClient(current_app.config['URI_ALODOKTER'])
        db = client.alodokter
        ...
        for post in db.core_posts.aggregate([{'$match': {"post_type": 'post', "post_status": 'publish'}}, {'$sample': {'size': 5} }]):
            ...
            if len(data_permalink) > 0:
                ...
                url = 'https://www.alodokter.com/'+ permalink
            else:
                url = 'https://www.alodokter.com/'+ post['post_name']
            ...
            if r.status_code == 200 and r.json()['responseCode'] == 200:
                ...
                # type: 5 = hospitals
                # type: 6 = doctors
                data = (uuid.uuid1().hex, 1, 1, u''+ url, score, rtime, datetime.datetime.now())
                rows_to_insert.append(data)
                current_app.logger.info('[AlodokterMonitor] '+ str(data))
        ...
        #         # type: 5 = hospitals
        #         # type: 6 = doctors
        #         data = (uuid.uuid1().hex, 1, 2, u''+ url, score, rtime, datetime.datetime.now())
        #         rows_to_insert.append(data)
        #         current_app.logger.info('[AlodokterMonitor] '+ str(data))
        ...
        #         # type: 5 = hospitals
        #         # type: 6 = doctors
        #         data = (uuid.uuid1().hex, 1, 3, u''+ url, score, rtime, datetime.datetime.now())
        #         rows_to_insert.append(data)
        #         current_app.logger.info('[AlodokterMonitor] '+ str(data))

        # # topik
        # for post in db.core_posts.aggregate([{'$match': {"post_type": 'topic', "post_status": 'publish'}}, {'$sample': {'size': 5} }]):
        #     data_permalink = [data for data in post['core_post_meta'] if data['meta_key'] == 'custom_permalink']
        #     if len(data_permalink) > 0:
        #         permalink = data_permalink[0]['meta_value']
        #         url = 'https://www.alodokter.com/komunitas/topic/'+ permalink
        #     else:
        #         url = 'https://www.alodokter.com/komunitas/topic/'+ post['post_name']
        ...
        #         # type: 5 = hospitals
        #         # type: 6 = doctors
        #         data = (uuid.uuid1().hex, 1, 4, u''+ url, score, rtime, datetime.datetime.now())
        #         rows_to_insert.append(data)
        #         current_app.logger.info('[AlodokterMonitor] '+ str(data))
